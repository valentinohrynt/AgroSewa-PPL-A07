...
<div class="left">
    ...
    <ul class="breadcrumb">
        ...
        <li>
            <a href="HalRiwayatPemerintah">Daftar Kelompok Tani</a>
        </li>
        <i class="fas fa-chevron-right"></i>
        <li>
...
<div class="box-info" style="display:flex; justify-content: flex-end;">
    <form id="form_{{ $lender->id }}" action="{{ route('HalRiwayatPengajuanBantuanPemerintah()') }}" method="post">
        @csrf
        <input type="hidden" name="lender_id" value="{{ $lender->id }}">
        <button type="submit" onclick="submitForm('{{ $lender->id }}')" class="btn btn-success" style="width: 15rem;">Pengajuan Bantuan</button>
    </form>
</div>
@endsection
